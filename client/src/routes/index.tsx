  ...
  head: () => ({
    meta: [
      { title: "GenWeb.ai" },
      {
        name: "description",
        content:
          "Build modern websites instantly using AI prompts. Generate responsive, production-ready websites with live preview and editable code.",
      },
      { property: "og:title", content: "GenWeb.ai — AI Website Builder" },
      {
        property: "og:description",
        content: "Turn ideas into beautiful websites in seconds. From prompt to production-ready.",
      },
    ],
  }),
  component: Landing,
...
function getGenerationErrorMessage(error: unknown) {
  if (!isAxiosError(error)) {
    return "Something went wrong while generating your website. Please try again.";
  }
  ...
  if (typeof message === "string") {
    ...
    if (normalized.includes("token") || normalized.includes("auth")) {
      return "Please sign in before creating a website.";
    }

    if (normalized.includes("credits")) {
      return "You do not have enough credits to generate a website.";
    }

    if (normalized.includes("prompt")) {
      return "Please describe the website you want to build.";
    }
  ...
  if (error.response.status >= 500) {
    return "The server could not generate your website right now. Please try again.";
  }
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-7xl px-6 pt-24 pb-20 text-center">
          ...
          >
            Turn ideas into beautiful <span className="text-gradient">websites in seconds.</span>
          </motion.h1>
          <motion.p
            initial={{ opacity: 0, y: 14 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            GenWeb.ai transforms simple prompts into fully responsive, production-ready websites
            with live preview and editable code.
          </motion.p>
          ...
          >
            <div id="prompt-section" className="glass rounded-2xl p-2 shadow-elegant">
              <div className="flex flex-col gap-3 rounded-xl bg-background/60 px-4 py-3 sm:flex-row sm:items-center sm:gap-2">
                ...
                <Button
                  ...
                  sx={{
                    ...
                    boxShadow: "0 8px 24px -8px rgba(139,92,246,.5)",
                  }}
                >
      ...
      <section className="border-y bg-surface">
        <div className="mx-auto max-w-7xl px-6 py-10 grid grid-cols-2 md:grid-cols-5 gap-6 items-center text-muted-foreground">
          {["Acme", "Nimbus", "Vector", "Forge", "GenWeb.ai"].map((n) => (
            <div
              key={n}
              className="text-center font-semibold tracking-tight opacity-70 hover:opacity-100 transition"
            >
              {n}
            </div>
          ))}
        ...
        <div className="mx-auto max-w-7xl px-6 py-24 grid lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <ul className="mt-8 space-y-3">
              {[
                "Change navbar color to violet",
                "Add a pricing section with 3 tiers",
                "Make it feel more like Linear",
              ].map((t) => (
          ...
          <div className="rounded-2xl border bg-card shadow-elegant p-5">
            ...
            <div className="space-y-3 text-sm">
              ...
              <ChatBubble role="ai">
                Updated the hero: softer radial gradient, larger display font, and a subtle floating
                screenshot. Want me to add testimonials next?
              </ChatBubble>
              ...
              >
                ...
                <Link to="/pricing">
                  <Button
                    ...
                    sx={
                      p.featured
                        ? { mt: 3, background: "linear-gradient(135deg,#6366f1,#a855f7)" }
                        : { mt: 3 }
                    }
                  >
                    Get started
                ...
                <ul className="mt-6 space-y-2.5 text-sm">
                  {p.features.map((f) => (
                    <li key={f} className="flex gap-2">
                      <Check size={16} className="text-[color:var(--accent)] mt-0.5" />
                      {f}
                    </li>
                  ))}
                </ul>
        ...
        <div className="relative overflow-hidden rounded-3xl border bg-card shadow-elegant p-12 md:p-16 text-center">
          ...
          <div className="relative">
            <h2 className="text-4xl md:text-5xl font-bold tracking-tight">
              Ready to build something <span className="text-gradient">beautiful</span>?
            </h2>
            <p className="mt-4 text-muted-foreground max-w-xl mx-auto">
              Start with a prompt. Ship in minutes.
            </p>
            <div className="mt-8 flex flex-wrap gap-3 justify-center">
              <a href="#prompt-section">
                <Button
                  variant="contained"
                  size="large"
              ...
              <Link to="/dashboard">
                <Button variant="outlined" size="large" startIcon={<Github size={16} />}>
                  Open dashboard
                </Button>
              </Link>
            </div>
...
const FEATURES = [
  {
    ...
    desc: "Generate complete sites — pages, components, copy — from one sentence.",
  },
  {
    icon: Code2,
    title: "Editable code",
    desc: "Real Monaco editor with multi-file support. Tweak, refactor, own it.",
  },
  {
    icon: Zap,
    title: "Instant preview",
    desc: "Hot-reloading sandbox renders your site in real time, across devices.",
  },
  {
    icon: MessageSquare,
    title: "AI partner",
    desc: 'Iterate via chat. "Make the hero bolder." "Add a CTA." Done.',
  },
  {
    icon: Palette,
    title: "Beautiful by default",
    desc: "Curated typography, color, and motion — no generic templates.",
  },
  {
    icon: Globe,
    title: "One-click export",
    desc: "Download a clean React/Vite project or deploy in a single click.",
  },
];
...
const TESTIMONIALS = [
  {
    ...
    quote:
      "GenWeb.ai replaced our design phase. We shipped a landing page in 40 minutes that looked better than our last agency build.",
  ...
  },
  {
    ...
    quote:
      "The code is genuinely clean. I exported and dropped it straight into our repo with zero rewrites.",
    color: "from-amber-400 to-rose-500",
  },
  {
    name: "Priya Raman",
    role: "Indie Hacker",
    quote: "Feels like pairing with a senior designer who actually types fast.",
...
const PLANS = [
  {
    ...
    featured: false,
    features: ["10 generations / mo", "Public projects", "Community support"],
  },
  {
    name: "Pro",
    price: "$24",
    ...
    featured: true,
    features: ["Unlimited generations", "Private projects", "Export to GitHub", "Priority AI"],
  },
  {
    name: "Team",
    price: "Custom",
    desc: "Collaboration for teams.",
    featured: false,
    features: [
      "Everything in Pro",
...
const FAQ = [
  {
    q: "How does GenWeb.ai generate websites?",
    a: "You describe the site, and our AI plans the structure, writes the code, and previews it live. You can refine via chat or edit the code directly.",
  },
  {
    q: "Do I own the code?",
    a: "Yes. Every project exports as a clean React + Vite codebase. There is no vendor lock-in.",
  },
  {
    q: "Can I deploy from GenWeb.ai?",
    a: "One-click deploy is available on Pro and Team plans, or export the ZIP and host anywhere.",
  },
  {
    q: "Is there a free tier?",
    a: "Yes — the Starter plan is free forever and includes 10 generations per month.",
  },
];
...
function ChatBubble({ role, children }: { role: "user" | "ai"; children: React.ReactNode }) {
  return (
    <div className={`flex ${role === "user" ? "justify-end" : "justify-start"}`}>
      <div
        className={`max-w-[85%] px-4 py-2.5 rounded-2xl text-sm ${role === "user" ? "bg-brand text-white" : "bg-muted"}`}
      >
        {children}
      </div>
    </div>
  ...
  return (
    <div className="rounded-2xl border bg-card shadow-elegant overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b bg-surface">
        ...
        <div className="text-xs text-muted-foreground glass px-3 py-1 rounded-md">
          GenWeb.ai/preview
        </div>
        <div className="flex gap-1">
          {[
            { i: Monitor, k: "desktop" },
            { i: Tablet, k: "tablet" },
            { i: Smartphone, k: "mobile" },
          ].map(({ i: Icon, k }) => (
            <button
              key={k}
              onClick={() => setDevice(k as any)}
              className={`p-1.5 rounded-md transition ${device === k ? "bg-muted text-foreground" : "text-muted-foreground hover:bg-muted/50"}`}
            >
              <Icon size={14} />
            </button>
          ))}
      ...
      <div
        className={`bg-surface overflow-x-auto flex justify-center ${device === "desktop" ? "p-0" : "p-6"}`}
      >
        <motion.div
          layout
        ...
        >
          ...
          <div className={`${preview.padding} bg-hero`}>
            <div className="inline-block px-2.5 py-1 text-[10px] uppercase tracking-widest text-[color:var(--accent)] border border-[color:var(--accent)]/30 rounded-full">
              v1.0
            </div>
            <div className={`mt-4 font-bold tracking-tight leading-tight ${preview.title}`}>
              Productivity, <span className="text-gradient">reimagined.</span>
            </div>
            <p className={`mt-2 text-muted-foreground ${preview.copy}`}>
              Ship beautiful interfaces in minutes with AI that understands taste.
            </p>
            <div className={`mt-5 flex gap-2 ${preview.buttons}`}>
              <div className="px-3 py-1.5 rounded-md bg-brand text-white text-xs font-medium">
                Start free
              </div>
